...
const { calcularValorReal } = require("../functions/calcularValor.js");
        ...
        for (const cfg of configs) {
            try {
                ...
                for (const ev of events.reverse()) {
                    ...
                    const matches = cfg.filter_type === "global" ||
                        (cfg.filter_type === "guild" && (ev.Killer?.GuildId === cfg.filter_id || ev.Victim?.GuildId === cfg.filter_id)) ||
                        (cfg.filter_type === "alliance" && (ev.Killer?.AllianceId === cfg.filter_id || ev.Victim?.AllianceId === cfg.filter_id)) ||
                        (cfg.filter_type === "player" && (ev.Killer?.Id === cfg.filter_id || ev.Victim?.Id === cfg.filter_id));
                    ...
                    const valor = await calcularValorReal(allVictimItems);
                    ...
                    const embed = {
                        ...
                        description: `**Dano do Killer:** ${percentDano}%\n**Valor estimado:** ${valor.toLocaleString("pt-BR")} silver`,
                        fields: [
